...
export const ClipPreviewModal: React.FC<ClipPreviewModalProps> = ({
  ...
  return (
    <div
      className="animate-in fade-in fixed inset-0 z-[150] flex items-center justify-center bg-black/70 p-6 backdrop-blur-sm duration-200"
      onClick={onClose}
    >
      <div
        className="animate-in zoom-in-95 flex h-full max-h-[85vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-white/10 bg-card/95 shadow-[0_0_50px_rgba(0,0,0,0.5)]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex flex-shrink-0 items-center justify-between gap-2 border-b border-white/5 bg-black/20 px-4 py-2.5">
          <div className="flex min-w-0 flex-1 items-center gap-2">
            <span className="truncate text-sm font-bold">
              {clip.source_app || clip.clip_type.toUpperCase()}
            </span>
            {detectedLang && (
              <span className="rounded border border-cyan-500/20 bg-cyan-500/10 px-1.5 py-0.5 font-mono text-[9px] uppercase text-cyan-300">
                {detectedLang}
              </span>
            )}
            ...
            {onEdit && isTextual && (
              <Tooltip label={t('contextMenu.edit')} placement="bottom">
                <button
                  ...
                  className="h-7 rounded-md px-2 text-white/50 transition-colors hover:bg-white/10 hover:text-white"
                >
                  <Code size={13} />
                </button>
              </Tooltip>
        ...
        {/* Content */}
        <div className="scrollbar-thin relative flex-1 overflow-auto bg-black/30 p-4">
          {clip.clip_type === 'image' ? (
            <div className="flex h-full items-center justify-center">
              {imageSrc ? (
                ...
                <span className="text-sm text-white/40">{t('common.image')}</span>
              )}
            </div>
          ) : clip.clip_type === 'file' ? (
            <div className="flex h-full flex-col items-center justify-center gap-3 text-center">
              <LucideFile size={36} className="text-white/20" />
              <div className="max-w-md break-all px-4 text-sm text-white/70">
                {(clip.preview || '').split('\n').map((line, i) => (
                  <div key={i}>{line}</div>
                ))}
              </div>
            ...
            <div className="relative h-full">
              {isHighlighting && (
                <div className="absolute right-2 top-2 z-10 flex items-center gap-1.5 text-[10px] text-cyan-300/70">
              ...
              {highlightedHtml ? (
                <pre
                  className={clsx(
                    'whitespace-pre-wrap break-words rounded-lg border border-white/5 bg-black/40 p-3',
                ...
                <pre className="whitespace-pre-wrap break-words font-mono text-[12px] leading-relaxed text-white/90">
                  {escapeHtml(content)}
                </pre>
              )}
            </div>
        ...
        {/* Footer actions */}
        <div className="flex flex-shrink-0 items-center justify-between border-t border-white/5 bg-black/20 px-4 py-2">
          ...
          <div className="flex gap-2">
            <button
              onClick={onClose}
              className="rounded-lg px-3 py-1.5 text-xs text-white/60 transition-colors hover:bg-white/10"
            >
              {t('common.close')}
            </button>
            {onEdit && isTextual && (
