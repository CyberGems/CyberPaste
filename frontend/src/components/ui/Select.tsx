...
export function Select({
  ...
  return (
    <div className={twMerge('relative w-full', className)} ref={containerRef}>
      ...
      {isOpen && (
        <div className="animate-in fade-in-0 zoom-in-95 absolute left-0 top-full z-50 mt-1 w-full overflow-hidden rounded-[6px] border border-border bg-popover text-popover-foreground shadow-lg duration-100">
          <div className="max-h-60 overflow-y-auto py-1">
            {options.map((option) => (
              <button
                ...
                className={clsx(
                  'relative flex w-full cursor-default select-none items-center py-1.5 pl-3 pr-8 text-[12px] outline-none transition-colors hover:bg-accent hover:text-accent-foreground',
